...
<script lang="ts">
  ...
  $effect(() => {
    if (open && menuEl) {
      ...
      if (rect.right > vw)   x = Math.max(0, x - (rect.right  - vw)  - 8);
      if (rect.bottom > vh)  y = Math.max(0, y - (rect.bottom - vh) - 8);
    }
  });
  ...
  // Inline SVG paths keyed by icon name (Lucide-compatible, 24×24)
